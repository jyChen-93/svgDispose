...
export default {
                    ...
                    list.forEach((shape) => {
                        ...
                        oldVal.forEach((old) => {
                            if (shape.shapeId == old.shapeId) {
                                has = true;
                                if (JSON.stringify(shape)!=JSON.stringify(old)) { //表明更新了数据
    ...
    methods: {
        ...
        updateShapeSeatStatus(shape, seats) { //更新shape里单个座位的状态渲染信息
            ...
            offCanvas.width = width * this.powNum;
            offCanvas.height = height * this.powNum;
            ...
            seats.forEach(seat => {
                ...
                if (shape.seats[j][i].status == 0) {
                    offContext.drawImage(this.seatIcon.iconShow, x * this.powNum, y * this.powNum, this.seatIcon.width * this.powNum, this.seatIcon.height * this.powNum);
                } else if (shape.seats[j][i].status == -1) {
                    offContext.drawImage(this.seatIcon.iconNone, x * this.powNum, y * this.powNum, this.seatIcon.width * this.powNum, this.seatIcon.height * this.powNum);
                }
            })

            let point1_1 = [center[0] - width / 2, center[1] - height / 2];
            ctx.save();
            ctx.translate(center[0] * this.powNum, center[1] * this.powNum);
            ctx.rotate(angle * Math.PI / 180);
            ctx.translate(-center[0] * this.powNum, -center[1] * this.powNum);
            ctx.drawImage(offCanvas, point1_1[0]* this.powNum, point1_1[1]* this.powNum, width * this.powNum, height * this.powNum);
            ctx.restore();
