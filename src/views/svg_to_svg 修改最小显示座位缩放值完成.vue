...
.warp
	...
	.content(ref="content")
		...
		div(v-for="(list,idx) in canvasList",:key="idx" v-show="wheelNum>=wheelShow")
...
export default {
    ...
    created() {
        ...
        this.seatIcon.width = 25 / this.scaleMax
        this.seatIcon.height = 25 / this.scaleMax
        this.seatIcon.marginRight = 7 / this.scaleMax
        this.seatIcon.marginBottom = 7 / this.scaleMax
        //计算出正方形对角线的长度
    ...
    methods: {
        ...
        comfrimSizeChange() { //确认修改画布大小
            ...
            this.clickToMax(this.checkedShape, this.wheelNum);
            //
            // //重新计算canvas的大小
            ...
            //     })
            // })
            // // canvas带下修改，修改重绘canvas，因此清空renderList
            // this.renderList = [];
            // this.clickToMax(this.checkedShape, this.wheelMax)
        },
            ...
            drawCanvas.forEach(canvasItem => {
                ...
                seats.forEach(seat => {
                    ...
                    if (shape.seats[j][i].status == 0) {
                        ...
                        canvasItem.ctx.beginPath();
                        canvasItem.ctx.drawImage(this.seatIcon.iconNone, real_x, real_y, this.seatIcon.width * this.wheelPow, this.seatIcon.height * this.wheelPow);
                    }
                })
